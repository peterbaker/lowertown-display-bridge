...
import { parseFilename, slotKey, dateStr, todayDow, isIgnored, IGNORE_REMEDY } from './filename.js';
...
export class Registry {
  ...
  /** All tier-4 (immediate) file paths — genuine `NOW-` files only. */
  immediateFiles() {
    return [...this.#files.entries()]
      .filter(([, m]) => m.tier === 4)
      .map(([p]) => p);
  ...
  /**
   * All registered files whose name matches no tier and is not a `NOW-` file.
   * These are tracked so the CLI can list them, and are never pushed.
   */
  ignoredFiles() {
    return [...this.#files.entries()]
...
/**
 * Render the `bridge.js schedule` Ignored section, or null when nothing is ignored.
 * The remedy string is the point of this section — see IGNORE_REMEDY.
 */
export function formatIgnoredSection(paths) {
  if (!paths || paths.length === 0) return null;
  const lines = [`  Ignored (never pushed) — ${IGNORE_REMEDY}:`];
  for (const p of [...paths].sort()) lines.push(`    ${basename(p)}`);
  return lines.join('\n');
}
