...
async function verifyPowerOn({ host, pin }) {
  ...
  try {
    ...
    if (!/\bon\b/i.test(output)) {
      console.debug(`[display] ⚠ Image sent but power state is "${output}" (expected ON) — display may be in standby`);
    }
  } catch (err) {
    // Don't escalate — sendImageDirect already succeeded and that's the
    // signal that matters most. Just leave a breadcrumb.
    console.debug(`[display] Post-push power probe failed: ${err.message.split('\n')[0]}`);
  }
}
