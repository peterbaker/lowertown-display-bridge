...
import { createPowerProbeGate } from './power-probe-gate.js';
...
// Self-disables on hardware that never answers MDC power queries (the
// EM32DX NAKs every one), so the probe can't spam the journal per push.
const powerProbeGate = createPowerProbeGate({ log: (msg) => console.warn(msg) });

async function verifyPowerOn({ host, pin }) {
  if (!SAMSUNG_MDC) return;
  if (!powerProbeGate.shouldProbe()) return;
  try {
    const [cmd, ...baseArgs] = mdcArgs({ host, pin });
    const output = await exec(cmd, [...baseArgs, 'power'], 8_000);
    powerProbeGate.recordSuccess();
    if (!/\bon\b/i.test(output)) {
      // This is the signal the probe exists for — a push that "succeeded"
      // onto a panel showing nothing. Worth a real warning.
    ...
    // signal that matters most. Just leave a breadcrumb, and stop leaving
    // one on every push if the panel never answers.
    powerProbeGate.recordFailure(err.message.split('\n')[0]);
  }
}
