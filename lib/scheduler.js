...
const GAP_MS = 10 * 60 * 1000;             // 10-minute minimum between scheduled pushes
...
export async function startBridge(config, { dryRun = false } = {}) {
  ...
  function warnConflictingSlots(hhmms) {
    ...
    for (let i = 0; i < sorted.length - 1; i++) {
      ...
      if (bMin - aMin < 10) {
        console.warn(
          `[bridge] ⚠ Slots ${sorted[i]} and ${sorted[i + 1]} are < 10 min apart — ` +
  ...
  async function handleMidnightRollover() {
    ...
    if (catchUp) {
      ...
      await sendNow(catchUp, { isScheduled: true });
    }
