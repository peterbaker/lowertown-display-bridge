...
import { wasApplicableAt, hhmmOf, dateStr, isIgnored, ignoreMessage } from './filename.js';
...
export async function startBridge(config, { dryRun = false } = {}) {
  ...
  if (ignoredAtStartup.length > 0) {
    console.log(
      `[bridge] Startup: ${ignoredAtStartup.length} file(s) ignored (unscheduled filenames) — ` +
      `'node bridge.js schedule' lists them`
    );
  }
