...
import { wasApplicableAt, hhmmOf, dateStr, isIgnored, ignoreMessage, IGNORE_REMEDY } from './filename.js';
...
export async function startBridge(config, { dryRun = false } = {}) {
  ...
  // The `add` handler names the remedy per file, but only for files that arrive
  // while the daemon is running. After a reboot with a stray file already in the
  // drop dir, the log would otherwise be silent about it — which is exactly the
  // case where someone is reading journalctl wondering why nothing is showing.
  // Count and a pointer, not per-file: a real drop dir holds a dozen T####
  // menus and per-file lines would bury the signal.
  const ignoredAtStartup = registry.ignoredFiles();
  if (ignoredAtStartup.length > 0) {
    // Carries the remedy *and* the pointer: the remedy is the fix, but at a
    // count granularity it doesn't say which files, so the pointer earns its
    // place. Same IGNORE_REMEDY as the per-file line, so the two can't drift.
    console.log(
      `[bridge] Startup: ${ignoredAtStartup.length} files ignored (unscheduled filenames) — ` +
      `${IGNORE_REMEDY}; 'node bridge.js schedule' lists them`
    );
  }
  ...
  watcher.on('add', async (filePath) => {
    ...
    // Unrecognised filename — registered so the CLI can list it, never pushed.
    // Dropping a file into the Drive folder must not be able to take the wall
    // over by accident (LOW-703); an immediate push is opt-in via `NOW-`.
    if (isIgnored(meta)) {
      console.log(`[bridge] ${ignoreMessage(name)}`);
      return;
    }
  ...
  watcher.on('change', async (filePath) => {
    ...
    registry.remove(filePath);
    const meta = registry.add(filePath);
    // Re-uploading the same name in Drive fires `change`, not `add`. An ignored
    // file has no slot so resolveCurrentDisplay() can never pick it, but say so
    // explicitly — this is the path that is easy to lose in a refactor.
    if (isIgnored(meta)) {
      console.log(`[bridge] ${ignoreMessage(basename(filePath))}`);
      return;
    }
  ...
  // Returned so tests (and any future embedder) can shut the daemon down
  // without signalling the process. systemd still stops us via SIGTERM.
  return {
    watcher,
    registry,
    async stop() {
