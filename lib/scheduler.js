...
export async function startBridge(config, { dryRun = false } = {}) {
  ...
  watcher.on('add', async (filePath) => {
    ...
    if (wasApplicableAt(meta, rightNow)) {
      // Slot has passed — push only if this is the cascade winner AND we
      // haven't just pushed this same file (guards against rclone re-delivery
      // of content-updated files, which chokidar reports as a fresh `add`).
