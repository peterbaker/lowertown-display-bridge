...
// Tier 4: explicit opt-in to an immediate full-screen takeover, case-insensitive.
// e.g. NOW-snow-day-closed.jpg  or  now closed.png
const IMMEDIATE_RE = /^NOW[-\s]/i;

/**
 * The remedy every ignore surface must carry — a misnamed file is otherwise a
 * silent no-op for whoever uploaded it, and this string is the whole fix.
 * Asserted literally in test/filename.test.js; do not reword it.
 */
export const IGNORE_REMEDY = 'rename with T####/NOW- to schedule it';

/** The daemon log line (sans '[bridge] ' prefix) for an ignored filename. */
export function ignoreMessage(name) {
  return `Ignored (unscheduled filename): ${name} — ${IGNORE_REMEDY}`;
}

/** True if this parsed descriptor is an unrecognised filename that must never be pushed. */
export function isIgnored(meta) {
  return Boolean(meta?.ignored);
}
 ...
 *   { tier: 3, hhmm: 'HHMM' }
 *   { tier: 4 }                  ← immediate (explicit 'NOW-' prefix only)
 *   { tier: null, ignored: true } ← unrecognised name; registered but never pushed
 */
export function parseFilename(filePath) {
  ...
  // Tier 4 — immediate, but only when the uploader explicitly asked for it.
  // Anything else is ignored: dropping a file into the Drive folder must not be
  // able to take the wall over by accident (LOW-703).
  if (IMMEDIATE_RE.test(stem)) return { tier: 4 };

  return { tier: null, ignored: true };
}

/**
 * The next Date this file should fire, relative to `from`.
 * Returns null for tier 1 if already past, and for tier 4 / ignored (no schedule).
 */
 ...
 * Returns null for tier 4 and ignored files (neither is slotted).
 */
