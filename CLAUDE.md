...
- **Fails closed on unknown names** — an unrecognised filename is registered, logged with the remedy, and never pushed, on `add` and on `change` alike; the startup scan logs a count of ignored files (not per-file) so a post-reboot stray isn't silent
...
- **The watcher is recursive, deliberately** — `chokidar.watch(dropDir, …)` in `lib/scheduler.js` passes no `depth`, and chokidar 4's default is unlimited. Combined with `rclone sync` mirroring the whole `Lowertown Display` tree, that is what makes a file in a *subfolder* of the drop dir fire exactly like one at the top level. The rule in the Architecture section above depends on all three links — the share puts the folder on the Pi, the recursive mirror puts the subfolder's file on disk, the unbounded watcher makes it fire. **Setting `depth` here would silently falsify that documented rule**; if you ever need to bound the watcher, fix the doc in the same commit.
